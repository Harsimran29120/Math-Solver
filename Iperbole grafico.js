function equationgraphMain() {
    this.version = '9';w = 470; h = 620; graphLt = 0; graphTp = 0; graphWd = 440; graphHt = 330; coords = new Coords(graphLt, graphTp, graphWd, graphHt, -5, -3, 5, 3, true); canvasWd = graphWd; canvasHt = graphHt; clrs = [["black", [0, 0, 125]], ["black", [255, 155, 0]]]; var s = ''; s += '<div style="position:relative; width:' + w + 'px; height:' + h + 'px; border-radius: 15px;  margin:auto; display:block; background-color: white; ">'; inits = ['x*x/3-y*y/2=1', 'y=x']; for (var i = 0; i < 2; i++) { s += '<input type="text" id="f' + i + '" style="position:absolute; left:10px; top:' + (6 + i * 40) + 'px; color: background-color: white; text-align:center; font: bold 20px Arial; width:350px; height: 40px ; border-radius: 10px;  z-index: 2;" value="' + inits[i] + '" onkeypress="onKeyPress(event)"/>'; s += '<button style="font: 16px Arial; position:absolute; height: 40px ; right:9px; top:' + (6 + i * 40) + 'px; z-index:2;" class="togglebtn"  onclick="doGraph()" >Go</button>'; }
    s += '<div style="position:absolute; left:95px; top:88px; color:black; font: 13px Arial; z-index: 2;">Zoom:</div>'; s += '<input  type="range" id="r1"  value="2" min="0" max="5" step=".01"  style=" background-color: beige ; border-radius: 20px ;  z-index:2; position:absolute; top:86px; left:130px; width:200px; height:17px; border: none; " oninput="onZoomChg(0,this.value)" onchange="onZoomChg(1,this.value)" />';s += '<canvas id="can" width="' + canvasWd + '" height="' + canvasHt + '" style="position:absolute; left:5px; top:42px; z-index:1; margin-top: 70px; margin-left: 5px; background-color: beige; border: 1px solid black;"></canvas>';   s += '<div id="linkpop" style="position:absolute; left:-450px; top:240px; padding: 5px; border: 1px solid red; border-radius: 9px; background-color: white; box-shadow: 10px 10px 5px 0 rgba(40,40,40,0.75); z-index:1; transition: all linear 0.3s; opacity:0; ">'; s += '</textarea >'; s += '<div style="float:right; margin: 0 0 5px 10px;">'; s += '<button onclick="linkPopClose()" style="z-index:2; font: 22px Arial;" class="togglebtn" >&#x2714;</button>'; s += '</div>'; document.write(s); el = document.getElementById('can'); ratio = 1; g = el.getContext("2d"); currZoom = 1; polarQ = false; aVal = 1; var inStr = getQueryVariable('func1'); if (inStr) { inStr = decodeURIComponent(inStr); document.getElementById('f0').value = inStr; }
